...
export default function Card({ card, isHighlighted, handler }: Props) {
    ...
    return (
        <div onClick={() => handler(card)} className={`group relative flex-shrink-0 p-0.5 rounded-md cursor-pointer smooth-transition z-[1] hover:-translate-y-1 ${isHighlighted && '-translate-y-3 hover:-translate-y-3'}`}>
